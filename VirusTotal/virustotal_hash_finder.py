#!/usr/bin/env python
# Return results for other hash types from VirusTotal, based on an input hash.
...
def main():
    ...
    try:
        ...
        sha1 = str(response_dict.get("sha1"))
        sha256 = str(response_dict.get("sha256"))
        md5 = str(response_dict.get("md5"))        

    except:
        m.returnOutput()
        
    if hash_type == 'md5':
        m.addEntity('malformity.Hash', sha1)
        m.addEntity('malformity.Hash', sha256)
        m.returnOutput()
    elif hash_type == 'sha1':
        m.addEntity('malformity.Hash', md5)
        m.addEntity('malformity.Hash', sha256) 
        m.returnOutput()
